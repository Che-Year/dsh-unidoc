...
.udc-ico{flex:none;font-size:12px}
...
`)
    ...
    const store = {
      ...
      rawPrefix: '',
      toasts: [],
      options: { codeEdit: true, mdPreview: true, unsupportedNotice: true },
      listeners: new Set(),
    }
    let toastSeq = 0
        ...
        for (const e of entries) {
          ...
          if (e.type === 'dir') {
            ...
            rows.push(React.createElement('div', {
              ...
              React.createElement('span', { className: 'udc-ico' }, fileIcon(extOf(e.name))),
              React.createElement('span', { className: 'udc-name' }, e.name),
    ...
    const fileIcon = (ext) => {
      ...
      if (OFFICE_EXT.has(ext) || ext === 'doc' || ext === 'ppt') return '📄'
